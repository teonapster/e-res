	...
	var isLoggedIn = function() {
	 ...
	 var login = function(username,password,callback){
		 return $http.post('rest/userAccess/login?u='+username+"&p="+password) // Umm.. encode the parameters?
		    .then(function(response){userChangeTo(response.data);},
		    function(){userChangeTo({});});
	 }
	 ...
	 var gotoUserHomePage = function() {
		 ...
		 if(status.user){
			 if(status.siteAdmin){
				 $location.path("/org");
			 } else if(status.orgAdmin){
				 $location.path("/org/"+status.orgid);				 
			 } else {
				 $location.path("/hosts");
			 } 
		 } else {
...
.controller('LoginController',['loginService','$scope','$location',
